      ...
      <nav className="mx-auto flex max-w-7xl items-center gap-3 rounded-3xl border border-glass-border bg-white/80 px-4 py-3 shadow-soft backdrop-blur-xl sm:gap-4 sm:px-6">
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 bg-white/70 py-2.5 pl-10 pr-4 text-sm text-slate-900 placeholder:text-slate-400 outline-none transition-colors focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
          />
        </div>
        ...
        >
          ...
          {totalItems > 0 && (
            <motion.span
              ...
              initial={{ scale: 0.4 }}
              animate={{ scale: 1 }}
              className="absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full bg-gradient-to-r from-blue-600 to-sky-500 px-1 text-[11px] font-bold text-white shadow-glow"
            >
              {totalItems}
            </motion.span>
          )}
